...
const onSubmit = async () => {
  ...
  if (resData?.data?._rawValue?.status == true) {
    ...
    handleClose();
    orderStore.getOneOrder(idOrder);
  } else {
    message.error({
      content: "Trả toàn bộ sách thất bại",
    });
  }
};
...
const props = defineProps({
  openReturnAll: Boolean,
  closeReturnAll: Function,
});
const open = ref(props.openReturnAll);
watch(
  () => props.openReturnAll,
  (newValue) => {
    open.value = newValue;
